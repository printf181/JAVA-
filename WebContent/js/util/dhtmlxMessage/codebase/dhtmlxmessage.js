...
(function () {
	...
	function e(a) {
		if (!e.cover) e.cover = document.createElement("DIV"), e.cover.onkeydown = j, e.cover.className = "dhx_modal_cover",
			document.body.appendChild(e.cover);
		var b = document.body.scrollHeight;
		e.cover.style.display =
			a ? "inline-block" : "none"
	}
	...
	function r(a) {
		if (!b.area) b.area = document.createElement("DIV"), b.area.className = "dhtmlx_message_area", b.area.style[b.position] =
			"5px", document.body.appendChild(b.area);
		b.hide(a.id);
		var d = document.createElement("DIV");
		d.innerHTML = "<div>" + a.text + "</div>";
		d.className = "dhtmlx-info dhtmlx-" + a.type;
		d.onclick = function () {
			b.hide(a.id);
			a = null
		};
		b.position == "bottom" && b.area.firstChild ? b.area.insertBefore(d,
			b.area.firstChild) : b.area.appendChild(d);
		a.expire > 0 && (b.timers[a.id] = window.setTimeout(function () {
			b.hide(a.id)
		}, a.expire));
		b.pull[a.id] = d;
		d = null;
	...
	function s(a, b, i) {
		...
		f += '<div class="dhtmlx_popup_text"><span>' + (a.content ? "" : a.text) +
			'</span></div><div  class="dhtmlx_popup_controls">';
		b && (f += l(a.ok || "OK", !0));
		i && (f += l(a.cancel || "Cancel", !1));
		if (a.buttons)
			for (var e = 0; e < a.buttons.length; e++) f += l(a.buttons[e], e);
		f += "</div>";
		c.innerHTML = f;
		if (a.content) {
			var h = a.content;
	...
	function m(a, b, i) {
		...
		var f = a.left || Math.abs(Math.floor(((window.innerWidth || document.documentElement.offsetWidth) - c.offsetWidth) /
				2)),
			g = a.top || Math.abs(Math.floor(((window.innerHeight || document.documentElement.offsetHeight) - c.offsetHeight) /
				2));
